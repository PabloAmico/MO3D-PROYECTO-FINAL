...
public class CameraControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if(Input.mousePosition.x < _mousePositionX)
            {
                //this.gameObject.transform.Rotate(0, _rotaton_Speed * Time.deltaTime, 0);
                transform.eulerAngles = new Vector3(60, transform.eulerAngles.y + _rotaton_Speed * Time.deltaTime, transform.eulerAngles.z);
            }
            else
            {
                if (Input.mousePosition.x > _mousePositionX)
